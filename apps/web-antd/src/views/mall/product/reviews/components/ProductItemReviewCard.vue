...
<template>
  <article
    class="grid grid-cols-1 gap-4 rounded-lg border border-border bg-background px-4 py-4 shadow-sm lg:grid lg:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_minmax(0,1fr)_12rem_5.5rem_3rem] lg:items-start lg:gap-6"
  >
    <div class="min-w-0">
    ...
    <div class="min-w-0 border-t border-border pt-3 lg:border-t-0 lg:pt-0">
      ...
      >
        <span>商品 ID：</span>
        <span class="text-foreground">{{ record.productId || '—' }}</span>
      </div>
    </div>
    ...
    <div
      class="flex min-h-[22px] items-center border-t border-border pt-3 lg:border-t-0 lg:pt-0"
    >
      <Button
        size="small"
        type="link"
