...
/** [menu-layout-custom] mixed-nav 且 Logo 保留在侧栏时，布局接近 sidebar-nav */
const isMixedNavWithSidebarLogo = computed(
  () => isMixedNav.value && props.mixedNavKeepSidebarLogo && !props.isMobile,
);
...
const sidebarMarginTop = computed(() => {
  ...
  if (isMixedNavWithSidebarLogo.value) {
    return 0;
  }
...
const mainStyle = computed(() => {
  ...
  if (
    ...
    (currentLayout.value !== 'mixed-nav' || isMixedNavWithSidebarLogo.value) &&
    currentLayout.value !== 'header-sidebar-nav' &&
    showSidebar.value &&
    !props.isMobile
  ) {
...
const tabbarStyle = computed((): CSSProperties => {
  ...
  // 如果不是混合导航，或 Logo 保留在侧栏，tabbar 的宽度为 100%
  if (
    !isMixedNav.value ||
    props.sidebarHidden ||
    isMixedNavWithSidebarLogo.value
  ) {
    width = '100%';
...
const headerWrapperStyle = computed((): CSSProperties => {
  ...
  return {
    ...
    left:
      isMixedNav.value && !isMixedNavWithSidebarLogo.value
        ? 0
...
<template>
  <div class="relative flex min-h-full w-full">
    <LayoutSidebar
      ...
      :header-height="isMixedNav && !mixedNavKeepSidebarLogo ? 0 : headerHeight"
    ...
    >
      <template
        v-if="isSideMode && (!isMixedNav || mixedNavKeepSidebarLogo)"
        #logo
      >
        <slot name="logo"></slot>
      </template>
