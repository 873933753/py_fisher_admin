...
function useMixedMenu() {
  ...
  const sidebarVisible = computed(() => {
    ...
    if (needSplit.value) {
      // [menu-layout-custom] 顶栏+侧栏模式下，无子菜单时也保留侧栏占位
      if (isTopSideMenuEnabled()) {
  ...
  const handleMenuSelect = (key: string, mode?: string) => {
    ...
    if (_splitSideMenus.length === 0) {
      navigation(key);
    } else if (isTopSideMenuEnabled()) {
      // [menu-layout-custom] 点击一级菜单时自动进入第一个子页面
      const firstChildPath = findFirstNavigableMenuPath(_splitSideMenus);
      if (firstChildPath) {
        navigation(firstChildPath);
      }
